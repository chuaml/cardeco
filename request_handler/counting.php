...
<form name="itemlistForm" method="POST" action="counting">
	...
	<select name="Select" required>
		...
		<option value="3column_by_itemCode__itemName">3columns Excel (by item code & name)</option>
	</select>
...
<?php
	...
	if ($_POST['Select'] === '3column_by_itemName') {
	...
	} else if ($_POST['Select'] === '3column_by_itemCode__itemName') {
		//3columns
		$data = trim($_POST['txtItems'], " \n\r\0\x0B"); //preserve \t tabs for fields delimiter
		$list = explode("\r\n", $data);
		//array_splice($list,0,1); //remove header
		 ...
		 * }> $itemList
		 * * Structure Details:
		 * - 0: Unique Item Identifier/SKU.
		 * - 1: Human-readable Item Name.
		 * - 2: Date string in 'yyyy-MM-dd' format.
		 * - 3: Integer status (e.g., 1 = Active, 0 = Inactive).
		 */
		$itemList = [
			// [
			// 	'id' => null,
		...
		for ($r = 0; $r < $len; ++$r) { // group by and count by xyz column
			// ( item code + item name ) as composite id
			$identifier = trim($list[$r][0]) . trim($list[$r][1]);
			if (array_key_exists($identifier, $itemList) === true) {
				$itemList[$identifier][3] += 1;
			} else {
				$itemList[$identifier] = [
					trim($list[$r][0]),
					trim($list[$r][1]),
